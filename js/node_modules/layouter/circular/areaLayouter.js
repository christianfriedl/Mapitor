...
AreaLayouter.prototype.layout = function() {
    ...
    var centeredWidget = this._node.map().widget(centeredNode);
    centeredWidget.moveTo(centerCoords.x, centerCoords.y);
    ...
    var dX = Math.max(0, (this._node.map().widget(this._node.map().area()).width() - window.mapitor.ui.domWidth()) / 2);
    var dY = Math.max(0, (this._node.map().widget(this._node.map().area()).height() - window.mapitor.ui.domHeight()) / 2);
    if ( dX > 0 || dY > 0 ) {
        this._node.map().scrollTo(dX, dY);
    }
};
