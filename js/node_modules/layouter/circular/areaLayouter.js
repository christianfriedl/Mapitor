...
AreaLayouter.prototype.layout = function() {
    console.log('using circular layouter for node', this._node);
    var area = this._node;
    ...
    // TODO currently we just assume that the currentNode is in our area, which is clearly bad
    //

    this._seenNodes = [];
    var currentNode = this._node.map().currentNode();
    this._layoutConnectedNodes(currentNode, 90, 90, 0, 360);
};
...
AreaLayouter.prototype._layoutConnectedNodes = function(/* m_node.Node */ p_node, /* number */ p_radius, /* number */ p_radiusDifference, /* number */ p_segmentStart, /* number */ p_segmentEnd) {
    this._seenNodes.push(p_node);
    var arrows = p_node.arrows();
    var targetNodes = this.collectTargetNodes(p_node, arrows, this._seenNodes);
    ...
    _(targetNodes).each(function(node) {
        var coords = m_trig.coordsOnCircle(this._node.map().currentNode().x(), this._node.map().currentNode().y(), p_radius, angle);
...
/*
 * TODO: extract to some static helper class
 */
AreaLayouter.prototype.collectTargetNodes = function(/* m_node.Node */ p_node, /* array(m_arrow.Arrow) */ p_arrows, /* array(m_node.Node) */ p_seenNodes) {
    var targetNodes = [];
    _(p_arrows).each(function(a) {
        var node = null;
