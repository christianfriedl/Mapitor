...
var m_keyboardEventHandler = require('ui/keyboardEventHandler.js');
...
function UI(/* dom element */ p_parent, /* m_map.Map */ p_map) {
    if ( arguments.length > 0 ) {
        ...
        this._map = p_map;
        this._keyboardEventHandler = m_keyboardEventHandler.keyboardEventHandler();
    }
};
...
UI.prototype.doTest = function() {
    ...
    for (i = 0; i < 3 + Math.random() * 50; ++i) {
        tns[i] = m_textNode.textNode(this._map, this._map.area(), { text: 'text (' + i + ')', preferredX: 180, preferredY: 180, width: 100, height: 40 });
        tns[i].text('textnode ' + tns[i].id());
        this._map.widget(tns[i]);
    }
    var a;
    var somanyarrows = 1 + Math.floor(Math.random() * tns.length);
    ...
    */
    ...
    this.layout();
    this.paint();
    this._paper.text(this._map._scrollX + 50,this._map._scrollY + 50, tns.length, { 'stroke': '#000', 'fill': '#000' });
};
...
/* void */ UI.prototype.dispatchKeyboardEvent = function(/* event */ ev) {
    var key = this._keyboardEventHandler.handle(ev);
    ...
    if ( key.char !== undefined ) {
        if ( key.ctrl ) {
            ...
            switch ( key.char ) {
                case m_keyboardEventHandler.Keys.tab:
                    if ( key.shift ) {
                    ...
                    break;
                case m_keyboardEventHandler.Keys.del:
                    m_deleteNodeCommand.deleteNodeCommand(this, this._map.focusedNode()).run();
                    break;
                case m_keyboardEventHandler.Keys.f2:
                    this.editTextNode(this._map.focusedNode());
                    break;
                default:
...
UI.prototype.editTextNode = function(p_node) {
    var painter = p_node.map().painter(p_node);
